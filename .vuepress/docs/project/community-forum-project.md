...
![image-20211202205832096](https://s2.loli.net/2022/04/09/8hvFnJR6IzgkcWM.png)
...
![image-20211205171140915](https://s2.loli.net/2022/04/09/i1DgbqhcyI7j2TL.png)
...
![image-20211207175157859](https://s2.loli.net/2022/04/09/rLPvtef3IG98Wzq.png)

AOP 的术语简介：

![image-20211207175417905](https://s2.loli.net/2022/04/09/PFovXLZ53NDde6V.png)
...
![image-20211209233832086](https://s2.loli.net/2022/04/09/PwHLdem7NMfOkVc.png)

![image-20211209233853389](https://s2.loli.net/2022/04/09/etWDsr9iQURunXF.png)
...
![image-20211211134320386](https://s2.loli.net/2022/04/09/5XREPFUDxy2YnA1.png)
...
![image-20211211141157162](https://s2.loli.net/2022/04/09/gOa4LnJ7AEZN5yp.png)
...
![image-20211211142356677](https://s2.loli.net/2022/04/09/K5Yu1FTwljoyQkW.png)
...
![image-20211211143753597](https://s2.loli.net/2022/04/09/jC6EKqrSu5xbTLw.png)
...
![image-20211211144551581](https://s2.loli.net/2022/04/09/iorw8UhqpsnQmS7.png)
...
![image-20211211145003316](https://s2.loli.net/2022/04/09/iLPFQqBnI2zas3p.png)
...
![image-20211211145219213](https://s2.loli.net/2022/04/09/kTZOoMwDFrRYdn4.png)

在此之后，我们在生产者中输入新的消息，消费者窗口就能同步查看到同样的消息。测试发现，目前只能直接发送英文消息，输入中文会造成编码错误，源于 GBK 字符集与 Unicode 字符集之间的转换问题。

![image-20211211145515643](https://s2.loli.net/2022/04/09/D7eEpdkFz9svAjg.png)
...
![image-20211211160746479](https://s2.loli.net/2022/04/09/ufcGLAgRrkDWUKO.png)
...
![image-20211211161235214](https://s2.loli.net/2022/04/09/hRbrOZF1WBUQHYP.png)

系统通知的形式比较相似，我们直接利用一个方法将所有类型的通知全部抽象起来。

![image-20211211163640619](https://s2.loli.net/2022/04/09/diYvGM8enAog2fm.png)
...
![image-20211212172323364](https://s2.loli.net/2022/04/09/rjkmCvHwpJZUPDF.png)
...
![image-20211212173106461](https://s2.loli.net/2022/04/09/ZQbB6juhz4VCacF.png)

IK 分词插件是根据 config 目录下的 dic 词典文件进行分词操作的，但是因为新词语的出现，原有的词典并不一定能满足所有需要。用户可以在 config 目录下的 IKAnalyzer.cfg.xml 文件中自己配置一些新的词语。

![image-20211212173359772](https://s2.loli.net/2022/04/09/UZTRPmy6rHaeNLC.png)
...
![image-20211213161924334](https://s2.loli.net/2022/04/09/2zrwgDmoqlNvaP8.png)

启动成功的运行状态：

![image-20211213162004956](https://s2.loli.net/2022/04/09/lhM6Fasp3yWnqvk.png)

可以从中看到，ES 默认占用端口 9200 。

由于已经调整 ES 占用内存为默认的 1G 内存，打开任务管理器可以看到非常高的内存占用。

![image-20211213162907391](https://s2.loli.net/2022/04/09/xaULWRGEiwdHVzT.png)
...
![image-20211213163302277](https://s2.loli.net/2022/04/09/OrpcjWhm74PEldi.png)
...
![image-20211213163413258](https://s2.loli.net/2022/04/09/xNyiX1BlLSqVOPe.png)
...
![image-20211213163542632](https://s2.loli.net/2022/04/09/yXAjkl1mCtbLPiW.png)
...
![image-20211213163740799](https://s2.loli.net/2022/04/09/dMci7DUytn5kjoJ.png)

再次输入命令查看索引，可以看到：

![image-20211213163816659](https://s2.loli.net/2022/04/09/o7Tz2C4AZ3p5bPY.png)
...
![image-20211213164019110](https://s2.loli.net/2022/04/09/kJygEAYhNoG5fQd.png)

同样，我们可以利用 postman 发送不同的 web 请求，包括 GET 、PUT 、 DELETE 等。测试的方法以及结果与命令行的结果一样，只是测试流程会更加简单。

![image-20211213164548612](https://s2.loli.net/2022/04/09/zh3NSTUOLwYFsqu.png)

发送创建索引的请求：

![image-20211213164610664](https://s2.loli.net/2022/04/09/17BjxFP4Ncusa2q.png)

发送删除索引的请求：

![image-20211213164637738](https://s2.loli.net/2022/04/09/gEfGpZjy3eb7LNr.png)

我们现在需要往索引中插入数据，指定提交的路径之后，我们添加 JSON 格式的字符串即可。

![image-20211213165159811](https://s2.loli.net/2022/04/09/BOgHQbVX1DpC4vr.png)
...
![image-20211213165530241](https://s2.loli.net/2022/04/09/b5RarteQd8vk1El.png)

删除数据与查询一致，只需要将请求方式修改为 DELETE 即可，返回的结果为删除操作成功与否的提示。

![image-20211213165724240](https://s2.loli.net/2022/04/09/M5yFwfx1EhIZvlN.png)

至此，对于 ES 的操作，我们可以大致了解到步骤：创建索引，之后向索引中添加索引，并且针对这些索引中的内容进行增删改查。

我们可以创建更多的 _doc 类型的数据，添加更多的文字信息。使用命令可以进行搜索指定关键字、指定位置（标题/内容）。

![image-20211213171025573](https://s2.loli.net/2022/04/09/ZHy4lAmhrp7Ue5b.png)
...
![image-20211213170509108](https://s2.loli.net/2022/04/09/G6Vex4uiENjfOJI.png)
...
![image-20211217225852201](https://s2.loli.net/2022/04/09/mBckfKDqbA6UeZM.png)
...
![image-20211218142907712](https://s2.loli.net/2022/04/09/BJgRTGaKUxX6ZEt.png)

转发：

浏览器向服务器发出请求，最终请求完成需要 A 和 B 组件一起协作完成，那么 A 组件会向 B 组件发起调用，会造成组件之间的耦合。整个过程是在一个请求之间完成的，在组件之间传参很容易实现。

![image-20211218143240196](https://s2.loli.net/2022/04/09/GQTntgYyvzw63pU.png)
...
![image-20211218154131203](https://s2.loli.net/2022/04/09/RvIqhVCTGywjuKf.png)
...
![image-20211218155458038](https://s2.loli.net/2022/04/09/OEonG7ZRDxMIjmL.png)
...
![image-20211218161037893](https://s2.loli.net/2022/04/09/anShgTMGR9UvBbx.png)

使用浏览器进行网络请求记录，查看 CSRF 令牌的传输，可以看到：

![image-20211218163522776](https://s2.loli.net/2022/04/09/TAOt2KZ54YzdcE7.png)
...
![image-20211220134944821](https://s2.loli.net/2022/04/09/OJVUpKMdrxRbsHY.png)
...
![image-20211220170050638](https://s2.loli.net/2022/04/09/HgUJw8ZyPpzo6jl.png)
...
![image-20211220181807924](https://s2.loli.net/2022/04/09/fjSsGTAxdctaeR4.png)
...
![image-20211220182148160](https://s2.loli.net/2022/04/09/q6kECFOjWMTluYQ.png)
